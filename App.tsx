...
const App: React.FC = () => {
  ...
  const handleAddNews = async (newItem: NewsItem) => {
    const loadingToastId = showLoading('Publicando notícia...');
    ...
    const payload = {
        ...
        date: new Date().toISOString().split('T')[0], // Use current date for DB
    ...
    // Update local state with the new item (including the ID generated by DB)
    const newNewsItem = { ...newItem, id: data[0].id, date: data[0].date };
    setNews([newNewsItem, ...news]);
    showSuccess('Notícia publicada com sucesso!');
  };

  const handleUpdateNews = async (updatedItem: NewsItem) => {
    const loadingToastId = showLoading('Atualizando notícia...');
  ...
  const handleDeleteNewsClick = async (id: number) => {
    const loadingToastId = showLoading('Excluindo notícia...');
