...
type SettingsSection = 'profile' | 'categories' | 'appearance' | 'security' | 'data';

const menuItems = [
  { id: 'profile', label: 'Dados Pessoais', icon: 'person', desc: 'Nome, Telefone e E-mail' },
  { id: 'categories', label: 'Categorias', icon: 'category', desc: 'Gerenciar Receitas e Despesas' },
  { id: 'appearance', label: 'Aparência', icon: 'palette', desc: 'Tema e Visual' },
  { id: 'security', label: 'Segurança', icon: 'lock', desc: 'Alterar Senha' },
  { id: 'data', label: 'Dados e Exportação', icon: 'cloud_download', desc: 'Backup e Exclusão de Conta' },
];
...
const SettingsPage: React.FC<SettingsPageProps> = ({ 
  ...
  // Email Verification State (Simplified for UI)
  ...
  const handleSaveProfile = async () => {
    ...
    if (profileForm.email !== user.email) {
        ...
        setIsSaving(false);
        // In a real app, this would trigger a Supabase email update flow
        showWarning(`Um código de verificação foi enviado para: ${profileForm.email}`);
        return;
    }

    // 2. Save Basic Info (Name/Phone) & CNPJ
    onUpdateUser({
        ...user,
    ...
    if (onCnpjChange) onCnpjChange(localCnpj);
  ...
  return (
     <div className="space-y-6 animate-in fade-in duration-500 pb-20 md:pb-8 flex flex-col md:flex-row gap-6 max-w-6xl mx-auto items-start">
        ...
        <div className="w-full md:w-64 flex-shrink-0 flex flex-col gap-2 md:sticky md:top-20 z-20 bg-background-light dark:bg-background-dark py-2 md:py-0">
        ...
        <div className="flex-1 w-full space-y-6">
           ...
           {activeSection === 'profile' && (
             <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 md:p-8 animate-in fade-in slide-in-from-right-4 duration-300">
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6">Dados Pessoais e CNPJ</h3>
           ...
           {activeSection === 'categories' && (
               <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 md:p-8 animate-in fade-in slide-in-from-right-4 duration-300">
                   <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6">Gerenciamento de Categorias</h3>
                   <CategoryManagement 
           ...
           {activeSection === 'appearance' && (
               <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 md:p-8 animate-in fade-in slide-in-from-right-4 duration-300">
                   <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6">Aparência e Tema</h3>
                   <p className="text-slate-500 dark:text-slate-400 mb-6">Escolha como você prefere visualizar a plataforma.</p>
           ...
           {activeSection === 'security' && (
               <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                   {onChangePassword && <ChangePasswordForm onChangePassword={onChangePassword} />}
               </div>
           )}

           {/* --- DATA SECTION (Export/Delete) --- */}
           {activeSection === 'data' && (
               <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 md:p-8 space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                   <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-4">Dados e Privacidade</h3>
