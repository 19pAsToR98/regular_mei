...
const TransactionQuickAddModal: React.FC<QuickAddModalProps> = ({
  type,
  isOpen,
  onClose,
  onSave,
  ...
  const [status, setStatus] = useState<'pago' | 'pendente'>(type === 'receita' ? 'pendente' : 'pago');
  ...
  useMemo(() => {
    ...
    if (isOpen) {
      ...
      setStatus(type === 'receita' ? 'pendente' : 'pago');
    }
  }, [isOpen, type]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!description.trim() || !category || isNaN(amountValue) || amountValue <= 0) {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="pt-2 flex gap-3">
            ...
            >
              Adicionar {isRevenue ? 'Receita' : 'Despesa'}
            </button>
          </div>
        </form>
      </div>
